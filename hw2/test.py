...
def testModel(config):
    ...
    binary_model.load_state_dict(torch.load(config['binary_model_path']))
    print('Testing using binary model from', config['binary_model_path'])
    ...
    multi_model.load_state_dict(torch.load(config['multi_model_path']))
    print('Testing using multi-class model from', config['multi_model_path'])
    ...
    with torch.no_grad():
        for i, pred in enumerate(predictions):
            if pred == 0:
                ...
                input_loader = DataLoader(input_dataset, batch_size=config['batch_size'], shuffle=False)
